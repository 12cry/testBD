...
//        t.testDisks();
    ...
    private void testDisks2() throws Exception {
        //1.初始化
//        SparkSession spark = Utils.getSpark();

        //2.查询数据
//		Dataset<Row> df = spark.read().format("jdbc")
//                .option("url", "jdbc:mysql://localhost:3306/test1?useUnicode=true&characterEncoding=UTF-8")
//				.option("dbtable", "t_p_host_disk_his").option("driver", "com.mysql.jdbc.Driver")
//                .option("user", "root").option("password", "root").load();
		Dataset<Row> df = sparkSession.read().format("jdbc")
                .option("url", "jdbc:mysql://172.16.3.223:3306/test?useUnicode=true&characterEncoding=UTF-8")
				.option("dbtable", "t_p_host_disk_his").option("driver", "com.mysql.jdbc.Driver")
...
//                .option("dbtable", "t_p_host_disk_his").option("driver", "oracle.jdbc.driver.OracleDriver")
//                .option("user", "portal").option("password", "gzcss").load();

        df.createTempView("t_p_host_disk_his");

        Dataset<Row> ds1 = sparkSession.sql("SELECT t.DISK_LOGIC_NAME name,unix_timestamp(t.COLLECTIONTIME)*1000 time,  " +
                "t.DISK_SPACE label, t.collectiontime time1 FROM t_p_host_disk_his t " +
                "where t.ci_code='10-2016-06-28' order by t.DISK_LOGIC_NAME");
        //10-2016-06-28

        System.out.println("共有数据条数："+ds1.count());
        ds1.show(100);
        ...
        ds1.javaRDD().mapToPair(row -> {
            ...
            list.add(map);
            return new Tuple2<>(row.getString(0), list);
        }).reduceByKey((l1, l2) -> {
            l1.addAll(l2);
            return l1;
        }).foreach(t -> {
            ...
            Dataset<Row> ds2 = sparkSession.createDataFrame(list, schema);
            //4.训练数据生成模型
            ...
            System.out.println("剩余误差:");
            trainingSummary.residuals().show(100);
            System.out.println("RMSE: " + trainingSummary.rootMeanSquaredError());
            System.out.println("r2: " + trainingSummary.r2());


            System.out.println("结果预测：");
            m.transform(ds2).show(100);
            double coefficients = m.coefficients().toArray()[0];
            if (coefficients != 0d) {
                System.out.print("预测硬盘过载时间（即硬盘剩余空间什么时候将为0）：");
    ...
    private void testDisks() throws Exception {
...
//				.option("dbtable", "t_p_host_disk_his").option("driver", "oracle.jdbc.driver.OracleDriver").option("user", "portal").option("password", "gzcss").load();
        ...
        df.createTempView("t_p_host_disk_his");
        Dataset<Row> ds1 = sparkSession.sql("SELECT t.DISK_LOGIC_NAME name,unix_timestamp(t.COLLECTIONTIME)*1000 time,  " +
                "t.DISK_SPACE label, t.collectiontime time1 FROM t_p_host_disk_his t where t.ci_code='10-2016-06-28'");
...
//				.option("dbtable", "t_p_host_drk();
        Dataset<Row> df = sparkSession.read().format("jdbc").option("url", "jdbc:oracle:thin:@172.16.3.223:1521:ORCL")
                .option("dbtable", "t_p_host_disk_his").option("driver", "oracle.jdbc.driver.OracleDriver").option("user", "portal").option("password", "gzcss").load();
        ...
        df.createTempView("t_p_host_disk_his");
        Dataset<Row> ds1 = sparkSession.sql(
                "SELECT unix_timestamp(t.COLLECTIONTIME)*1000 x,  t.DISK_SPACE label FROM t_p_host_disk_his t " +
                        "WHERE t.CI_CODE = '10-2016-06-28' AND t.DISK_LOGIC_NAME = '/dev/mapper/rootvg-homelv'");
        ...
        Dataset<Row> testDS = sparkSession.createDataFrame(Arrays.asList(RowFactory.create(Vectors.dense(DateUtils.addDays(new Date(), -1011).getTime()))),
